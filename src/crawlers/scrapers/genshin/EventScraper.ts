...
export class GenshinEventScraper extends ScraperBase {
  private readonly API_URL =
    'https://bbs-api-os.hoyolab.com/community/post/wapi/getNewsList?gids=2&page_size=15&type=1';
  ...
  async scrape(): Promise<ScrapedData[]> {
    logger.info('Starting Genshin Event scraping...');
    ...
    try {
      ...
      if (response.data.retcode !== 0) {
        ...
        throw new Error(`Genshin event API error: ${response.data.message}`);
      }
  ...
  async save(data: ScrapedData[]) {
    const outputDir = path.join(process.cwd(), 'data', 'crawlers', 'genshin');
